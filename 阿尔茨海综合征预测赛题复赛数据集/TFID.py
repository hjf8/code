# -*- coding: utf-8 -*-
"""
Created on Thu Sep 12 10:51:29 2019
...
def one_hot_col(col):
    ...
    lbl.fit(col)
    return lbl
def xgb_model(new_train,y,new_test,N):
  '''定义模型'''
  xgb_params = {
          'booster': 'gbtree',
  ...
  oof_xgb=np.zeros((new_train.shape[0],3))
  prediction_xgb=np.zeros((new_test.shape[0],3))
  for i,(tr,va) in enumerate(skf.split(new_train,y)):
    print('fold:',i+1,'training')
    dtrain = xgb.DMatrix(new_train.loc[tr],y[tr])
    dvalid = xgb.DMatrix(new_train.loc[va],y[va])
    ...
    oof_xgb[va] += bst.predict(xgb.DMatrix(new_train.loc[va]), ntree_limit=bst.best_ntree_limit)
    prediction_xgb += bst.predict(xgb.DMatrix(new_test), ntree_limit=bst.best_ntree_limit)
  print('1')
  prediction_xgb/=N
  return np.argmax(prediction_xgb, axis=1),prediction_xgb
...
for path in tqdm(tsv_path_lists): ##遍历每个文件，提取特征
    ...
    trn1=trn.describe()
    trn2=np.max(trn1.loc['mean'],axis=0)
    tsv1 = analysis_df(z)
...
'''
prediction_lgb2,lgb12=\
      xgb_model(preliminary_list_train.drop(['uuid','label'],axis=1),\
                preliminary_list_train['label'],\
                preliminary_list_test.drop(['uuid','label'],axis=1),\
                10)
'''
preliminary_list_test['label']=prediction_lgb1
