# -*- coding: utf-8 -*-
"""
Created on Sun Jan  6 14:54:25 2019
...
train=train.sort_index(by='id').reset_index(drop=True)
train.drop(['id'],axis=1,inplace=True)
test['id']=test['样本id'].apply(lambda x:int(x[7:]))
test=test.sort_index(by='id').reset_index(drop=True)
...
for f in categorical_columns:
    col_name = f+"_收率"+'_mean'
    mean_features.append(col_name)
    order_label = train.groupby([f])['target'].mean()
    for df in [train, test]:
            df[col_name] = df[f].map(order_label)     
'''
