# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 19:05:20 2019
...
def get_feature(wav_file,Lens = 12,BASE_DIR=BASE_DIR):
   ...
   # print('==========',normalize(normalize(feature)))
...
def create_csv(TXT_DIR=TXT_DIR):
    ...
    lists = lists.sample(frac=1)#随机打乱顺序
...
if __name__ == "__main__":
    ...
    plt.show()"""
    '''
    xx=np.load('preliminary/feature_single08-Apr-2019.npy')
    label=np.load('preliminary/label08-Apr-2019.npy')
    z=[i for i in range(len(label))]
    np.random.shuffle(z)
    xx=np.array([xx[i] for i in z])
    label=label[z]
    xx=xs_gen(xx)
    '''
    ...
    #model.fit(xx, label, epochs=50, batch_size=128)
